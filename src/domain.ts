...
export interface Product {id:string;name:string;category:number;price:number;cost:number|null;unit:string}
export interface Line {id:string;productId:string;quantity:number;price:number;discount:number;cost:number|null;originalLineId?:string}
export interface Receipt {id:string;storeId:string;customerId:string|null;at:string;type:'sale'|'return';originalId?:string;lines:Line[]}
export interface Customer {id:string;birthDate:string|null;consent:boolean;channel:boolean;contacts:number;storeId:string;receipts:Receipt[]}
...
export const fields:Record<Field,{label:string;ops:string[]}>={count:{label:'Число чеков',ops:['gte','lte','eq']},spend:{label:'Сумма покупок, ₽',ops:['gte','lte']},days:{label:'Дни с покупками',ops:['gte','lte']},average:{label:'Средний чек, ₽',ops:['gte','lte']},last:{label:'Дней с покупки',ops:['gte','lte']},category:{label:'Категория',ops:['yes','not']},weekday:{label:'Чеки в пт/сб, %',ops:['gte','lte']},store:{label:'Основной магазин',ops:['eq']},consent:{label:'Согласие',ops:['yes','not']},channel:{label:'Доступен канал',ops:['yes','not']},contacts:{label:'Получено предложений',ops:['gte','lte']},age:{label:'Известный возраст',ops:['gte','lte']}};
...
export function offerErrors(o:Offer,p:Product|undefined,n:number,products?:Product[]){const errors:string[]=[];if(!p)errors.push('Товар не найден');else {const e=economy(p,o);if(p.cost===null)errors.push('Неизвестна себестоимость товара');else if(e.margin===null||e.margin<o.minMargin)errors.push(`Маржа ${num(e.margin||0,2)}% ниже минимальной ${o.minMargin}%`);}if(o.category!==null&&products){const bad=products.filter(p=>p.category===o.category&&(p.cost===null||(economy(p,o).margin??-1)<o.minMargin));if(bad.length)errors.push(`В категории ${bad.length} товаров с неизвестной себестоимостью или недостаточной маржей`);}if(n<=0)errors.push('Нет доступных получателей');if(!o.stores.length)errors.push('Выберите магазин');if(o.stores.every(s=>s==='S12'))errors.push('В выбранных магазинах нет товара');if(!o.from||!o.to||o.from>o.to)errors.push('Проверьте даты начала и окончания');if(o.from&&o.to&&dayStart(o.to)-dayStart(o.from)>=90*DAY)errors.push('Срок кампании ограничен 90 днями');if(!o.weekdays.length||o.hourFrom>=o.hourTo||o.hourFrom<0||o.hourTo>24)errors.push('Проверьте дни и часы действия');if(o.value<=0||o.kind==='percent'&&o.value>100||o.cap<=0||o.minBasket<0)errors.push('Скидка, лимит и сумма корзины некорректны');if(o.control<=0||o.control>=100)errors.push('Контрольная группа должна быть от 1 до 99%');if(n*o.cap+n*o.channelCost>o.budget)errors.push(`Бюджет меньше обязательства ${money(n*o.cap+n*o.channelCost)}`);return errors;}
...
export interface Design {schemaVersion:1;brandId:string;brandVersion:number;templateId:string;asset:string;title:string;description:string;fontSize:number;align:'left'|'center';crop:{x:number;y:number;scale:number};showDescription:boolean;reverse:boolean;revision:number;variantId?:string;logo?:string;brandTokens?:{primary:string;ink:string;paper:string;font:string;pattern:string;fontUrl?:string}}
export const defaultDesign:Design={schemaVersion:1,brandId:'kvartal',brandVersion:1,templateId:'right',asset:'/assets/kvartal-right.png',title:'на любимые\nснеки',description:'Больше вкусных\nмоментов рядом',fontSize:28,align:'left',crop:{x:50,y:50,scale:1},showDescription:true,reverse:false,revision:1};
export const themes=[{id:'kvartal',name:'Квартал 12',primary:'#F25835',ink:'#191B1D',paper:'#F6F0E8',font:'Inter',asset:'/assets/kvartal-right.png'},{id:'copper',name:'Медный двор',primary:'#173D32',ink:'#F3E4BA',paper:'#F5F0E3',font:'Georgia',asset:'/assets/copper.png'},{id:'lemon',name:'Лимон & соль',primary:'#F2DB38',ink:'#191B1D',paper:'#FFFCDF',font:'Inter',asset:'/assets/lemon.png'}];
export interface Assignment {id:string;customerId:string;group:'offer'|'control';token:string;used:boolean}
export interface Campaign {id:string;name:string;status:'draft'|'scheduled'|'active'|'paused'|'completed'|'archived';step:number;rules:Group;offer:Offer;design:Design;assignments:Assignment[];personalId?:string;snapshot?:{offer:Offer;design:Design;rules:Group};created:string}
...
export function campaignsSeed():Campaign[]{return[{id:'CP-104',name:'Вернуть в будни',status:'draft',step:0,rules:structuredClone(baseSegment),offer:{...defaultOffer},design:{...defaultDesign},assignments:[],created:DEMO_NOW},{id:'CP-105',name:'Вернуться за любимым',status:'draft',step:1,rules:{id:'root',logic:'AND',children:[{id:'r1',field:'last',op:'gte',value:21,window:90}]},offer:{...defaultOffer,kind:'fixed',value:50,minBasket:50000,productId:'P004'},design:{...defaultDesign,asset:'/assets/kvartal-air.png',title:'на новые\nвпечатления'},assignments:[],created:DEMO_NOW},{id:'CP-097',name:'Попробовать новое',status:'completed',step:4,rules:structuredClone(baseSegment),offer:{...defaultOffer,from:'2026-08-17',to:'2026-09-13',weekdays:[0,1,2,3,4,5,6]},design:{...defaultDesign,asset:'/assets/kvartal-scene.png',title:'откройте\nновый вкус'},assignments:[],created:DEMO_NOW}];}
